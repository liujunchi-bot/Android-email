...
public class ReadInnerMailActivity extends AppCompatActivity {    //收件箱
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SimpleAdapter simpleAdapter = new SimpleAdapter(this,mailList,R.layout.mail_item,
                new String[]{"from","time","content"},new int[]{R.id.from,R.id.time,R.id.title});
...
//                Toast.makeText(getApplicationContext(), "第"+position+"个项", Toast.LENGTH_SHORT).show();
                ...
                Intent intent = new Intent(ReadInnerMailActivity.this, CheckMailActivity.class);

                Map<String,Object> map = new HashMap<String,Object>();
                map = mailList.get(position);
                intent.putExtra("from",map.get("from").toString());
...
//            Toast.makeText(getApplicationContext(), "账号为空", Toast.LENGTH_SHORT).show();
            Toast.makeText(getApplicationContext(), "收件箱为空", Toast.LENGTH_SHORT).show();
        }
        else {
            //连接服务器，读取收件
            new Thread(new ReadInnerMailActivity.MyThread()).start();
...
//        list.add(map3);
    ...
    public class MyThread implements Runnable{
        @Override
        public void run() {
            System.out.println("收件箱获取开始了");

            mailListString = ReadInnerMail.readInnerMail(user);
